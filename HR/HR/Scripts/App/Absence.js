(function (jQuery) {
    ...
    var daterangepickerOptions = {
        ...
        minDate: jQuery('#EmploymentBeginDate').data("beginDate"),
        maxDate: jQuery('#EmploymentEndDate').data("endDate"),
        opens: 'left',
        locale: {
            format: 'DD MMMM YYYY'
        }
    ...
    function AbsenceRequest() {
        var postData = {
            ...
            PublicHolidayPolicyId: jQuery("#Absence_PublicHolidayPolicyId").val(),
            ...
            //AbsenceStatusId: jQuery("#Absence_AbsenceStatus_AbsenceStatusId").val(),
        };
            ...
            .done(function (response) {
                if (!jQuery.isEmptyObject(response)) {
                    ...
                    if (response.PersonnelAbsenceEntitlement.Entitlement) {
                        ...
                        var afterbooking = response.PersonnelAbsenceEntitlement.Remaining - response.Duration;

                        jQuery("#submit").prop("disabled", function () {
                            return afterbooking < 0 || afterbooking == response.PersonnelAbsenceEntitlement.Remaining;
                        });
                    ...
                    //Absence Type based on begindate
                    jQuery("#Absence_AbsenceTypeId").empty();
                    jQuery.each(response.AbsenceTypes, function (key, value) {
                        $("#Absence_AbsenceTypeId").append($("<option></option>").val(value.AbsenceTypeId).html(value.Name));
                    });
                    jQuery("#Absence_AbsenceTypeId").val(response.AbsenceType.AbsenceTypeId).attr("selected", "selected");
                 
                }
                jQuery("#submit").prop("disabled", function () {
                    return afterbooking < 0 || afterbooking == response.PersonnelAbsenceEntitlement.Remaining;
                });
                if(!checkCanBeBookedAsAbsence(response)) {
              ...
              .html("<ul><li>Unable to find the Employment.</li></ul>")
               .show();
                jQuery("#employmentError")
                    .parent()
                    .closest(".row")
    ...
    function checkCanBeBookedAsAbsence(response) {
        var result = false;
        jQuery.each(response.AbsenceDays, function (i, item) {
            if (item.CanBeBookedAsAbsence) {
                result = true;
                return result;
            }
        });
